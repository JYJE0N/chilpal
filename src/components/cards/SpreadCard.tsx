...
export default function SpreadCard({
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className={`glass-card-light p-6 cursor-pointer border-2 ${
        colors.border
    ...
    >
      ...
      {/* 설명 - 노란색 텍스트 */}
      <p className="text-yellow-200 text-sm mb-4">{spread.description}</p>

      {/* 카드 수만 표시 */}
      <div className="mb-4">
        <div className="flex items-center justify-between text-sm">
          <span className="text-white/70">Draw:</span>
          <span className="text-white font-medium">{spread.cardCount}장</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {spread.recommendedFor.slice(0, 3).map((item, index) => (
          <span
            key={index}
            className="text-xs bg-cyan-500/20 text-cyan-200 px-3 py-1 rounded-full border border-cyan-500/30"
          >
            {item}
          </span>
        ))}
